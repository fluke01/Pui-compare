...
def main(page: ft.Page):
    ...
    def show_result(rows, winner_names, cheapest_price):
            ...
            result_content.controls.append(
                ft.Container(
                    content=ft.Row([
                        ...
                        ft.Column([
                            ft.Text(name, weight="bold" if is_winner else "normal",
                                    size=14, color="#1A237E" if is_winner else "#455A64"),
                            ft.Text(f"{unit_price:.2f} บาท/หน่วย", size=13,
        ...
        result_content.controls.append(
            ft.Container(
                content=ft.Text(f"✅ คุ้มค่าที่สุด: {winner_label}",
                                weight="bold", size=14, color="#2E7D32"),
    ...
    def calculate_result(e=None):
        ...
        page.update()
        rows_with_flag = [(n, p, n in cheapest_names) for n, p in result_rows]
        show_result(rows_with_flag, cheapest_names, cheapest_price)

    def add_entry(e=None):
        idx = len(entries) + 1

        name = ft.TextField(label="ชื่อสินค้า", hint_text="เช่น น้ำยาล้างจาน A",
                            expand=True, border_radius=8, focused_border_color="#3949AB")
        price = ft.TextField(label="ราคา (บาท)", keyboard_type=ft.KeyboardType.NUMBER,
                             expand=True, border_radius=8, prefix_icon=ft.icons.ATTACH_MONEY,
                             focused_border_color="#3949AB")
        quantity = ft.TextField(label="ปริมาณ", keyboard_type=ft.KeyboardType.NUMBER,
                                expand=True, border_radius=8, prefix_icon=ft.icons.SCALE,
                                focused_border_color="#3949AB")
        unit_price = ft.Text(value="", size=12, color="#7986CB", italic=True)

        row_data = {"name": name, "price": price, "quantity": quantity, "unit_price": unit_price}

        delete_btn = ft.IconButton(
            icon=ft.icons.DELETE_OUTLINE, icon_color="#EF5350", tooltip="ลบสินค้า",
        ...
        entry_container = ft.Container(
            content=ft.Column([
                ft.Row([
                    ft.Row([ft.Icon(ft.icons.SHOPPING_BASKET_OUTLINED, color="#3949AB", size=18),
    ...
    action_row = ft.Container(
        content=ft.Row(
            [
                ft.ElevatedButton("เพิ่มสินค้า", icon=ft.icons.ADD_CIRCLE_OUTLINE,
                                  on_click=add_entry,
                                  style=ft.ButtonStyle(bgcolor="#E8EAF6", color="#3949AB",
                                                       padding=ft.padding.symmetric(horizontal=20, vertical=14),
                                                       shape=ft.RoundedRectangleBorder(radius=12))),
                ft.FilledButton("คำนวณ", icon=ft.icons.CALCULATE_OUTLINED,
